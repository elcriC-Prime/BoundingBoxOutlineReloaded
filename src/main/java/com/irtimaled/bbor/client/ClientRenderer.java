...
public class ClientRenderer {
    ...
    private static final Set<IBoundingBoxProvider> providers = new java.util.concurrent.CopyOnWriteArraySet<>();
    ...
    public static void toggleActive() {
        ...
        if (active) {
            Player.setActiveY();
            AsyncRenderer.forceRebuild();
        }
        MinecraftClient client = MinecraftClient.getInstance();
        if (client != null && client.player != null) {
            String status = active ? "§a" + I18n.translate("bbor.options.active") : "§c" + I18n.translate("gui.none");
            client.player.sendMessage(Text.literal("§6[BBOR] §r" + status), true);
        }
    }
    ...
    public static void render(MatrixStack matrixStack, Matrix4f modelViewMatrix, Matrix4f projectionMatrix, net.minecraft.client.render.Camera camera, DimensionId dimensionId) {
        AsyncRenderer.render(matrixStack, modelViewMatrix, projectionMatrix, camera, dimensionId);
    }
