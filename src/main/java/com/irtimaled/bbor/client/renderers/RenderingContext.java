...
public class RenderingContext {
    ...
    public void doDrawing(Matrix4f modelViewMatrix, Matrix4f projectionMatrix) {
        long startTime = System.nanoTime();

        RenderSystem.depthMask(true);
        if (!lineBufferUploadedEmpty) {
            lineBufferUploaded.bind();
            lineBufferUploaded.draw(modelViewMatrix, projectionMatrix, GameRenderer.getPositionColorProgram());
        }
        if (!quadBufferMaskedUploadedEmpty) {
            quadBufferMaskedUploaded.bind();
            quadBufferMaskedUploaded.draw(modelViewMatrix, projectionMatrix, GameRenderer.getPositionColorProgram());
        }

        RenderSystem.depthMask(false);
        if (!quadBufferNonMaskedUploadedEmpty) {
            quadBufferNonMaskedUploaded.bind();
            quadBufferNonMaskedUploaded.draw(modelViewMatrix, projectionMatrix, GameRenderer.getPositionColorProgram());
        }
