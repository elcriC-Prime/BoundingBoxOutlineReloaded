...
public class CommonInterop {
    ...
    public static void loadWorlds(Iterable<ServerWorld> worlds) {
    ...
    public static void loadStructuresFromRegistry(Registry<Structure> structureFeatureRegistry) {
        Logger.info("BBOR: Registering structures from registry: %s", Arrays.toString(structureFeatureRegistry.getEntrySet().stream().map(entry -> entry.getKey().getValue().toString()).distinct().toArray(String[]::new)));
    ...
    public static void loadStructuresInitial() {
        final List<RegistryKey<Structure>> keys = BuiltinRegistries.createWrapperLookup().getWrapperOrThrow(RegistryKeys.STRUCTURE).streamKeys().toList();
        Logger.info("BBOR: Registering structures initial: %s", Arrays.toString(keys.stream().map(key -> key.getValue().toString()).distinct().toArray(String[]::new)));
        for (var key : keys) {
